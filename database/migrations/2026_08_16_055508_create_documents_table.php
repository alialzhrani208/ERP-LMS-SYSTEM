...
return new class extends Migration
{
    ...
    $table->string('status')->default('active')->index(); // هذا هو الحقل في جدول documents
    $table->foreignId('category_id')->constrained()->cascadeOnDelete(); // التصنيف
    $table->string('attachment')->nullable(); // ملف الوثيقة المرفق
    $table->date('document_date')->nullable()->index(); // تاريخ الوثيقة
    $table->date('expiry_date')->nullable()->index(); // تاريخ انتهاء الوثيقة (للعقود والتراخيص)
